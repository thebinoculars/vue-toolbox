  ...
  <div class="flex flex-col h-full">
    <div class="flex-1 overflow-hidden">
      <div class="h-full overflow-hidden p-4 flex flex-col lg:flex-row gap-4">
        <!-- Preview Area -->
        <div
          class="flex-1 rounded-2xl border overflow-hidden flex flex-col bg-[var(--bg-secondary)] border-[var(--border-color)]"
        >
          <input
            ref="fileInput"
            type="file"
          ...
          <div
            ...
            class="flex-1 flex items-center justify-center cursor-pointer bg-opacity-50 transition-all overflow-auto scrollbar-hide bg-[var(--bg-primary)]"
        ...
        <!-- Settings Panel -->
        <div
          class="w-full lg:w-96 rounded-2xl border p-4 flex flex-col overflow-y-auto bg-[var(--bg-secondary)] border-[var(--border-color)]"
        >
          <div class="space-y-4">
            <div class="space-y-3">
              ...
              <textarea
                ...
                rows="10"
                class="w-full rounded-xl border p-3 text-sm font-mono resize-none outline-none bg-[var(--bg-primary)] text-[#e5e5e5] border-[var(--border-color)]"
...
import { Download, Photo, PlayerPlay, Upload } from '@vicons/tabler'

interface TextArtSettings {
  width: number
  height: number
  lyrics: string
...
interface TextArtCanvasContext2D {
  fillStyle: string
...
interface TextArtCanvasLike {
  width: number
  height: number
  getContext(
    contextId: string,
    options?: { willReadFrequently: boolean },
  ): TextArtCanvasContext2D | null
}
...
const generateTextPoster = async (
  ...
  return new Promise<void>((resolve, reject) => {
    const canvasEl = canvas as TextArtCanvasLike
    ...
    img.onload = () => {
      ...
      if (!offCtx) {
        return reject(new Error('Could not get offscreen context'))
      }
