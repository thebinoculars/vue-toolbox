  ...
  <div class="w-full max-w-md px-4">
    <n-card :bordered="false" class="shadow-xl rounded-2xl">
      <div class="text-center mb-8">
        <n-icon size="48" color="#6366f1" class="mb-3"><UserPlus /></n-icon>
      ...
      <n-form @submit.prevent="handleRegister">
        ...
        <n-alert v-if="success" type="success" class="mb-4" :bordered="false">{{
          success
        }}</n-alert>
      ...
      <div class="text-center space-y-2">
        ...
        <div>
          <router-link to="/">
            <n-button text size="small" class="text-[var(--icon-color)]">← Back to Home</n-button>
          </router-link>
        </div>
      </div>
    </n-card>
...
const handleRegister = async () => {
  ...
  try {
    const { data, error: errorMessage } = await authStore.register(email.value, password.value)
    if (!data?.success) {
      error.value = errorMessage || 'Register failed'
      return
    }
