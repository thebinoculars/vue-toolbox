      ...
      >
        ...
        >
          <div
            ...
            @click="openSlider(images.findIndex((img) => img.id === image.id))"
          >
...
import { Download, InfoCircle, PhotoOff, Refresh, Trash, Upload } from '@vicons/tabler'
  ...
  try {
    for (const file of validFiles) {
      try {
        ...
        await albumsRepository.uploadImage(props.albumId, formData)
        successCount++
      } catch {
...
const handleDeleteImage = (imageId: number) => {
  const dialogInstance = dialog.warning({
    ...
    onPositiveClick: async () => {
      ...
      try {
        await albumsRepository.deleteImage(props.albumId, imageId)
