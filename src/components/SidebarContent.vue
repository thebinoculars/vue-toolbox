<template>
  <div class="p-2 border-b flex items-center gap-2"
    ...
    <n-input :value="search" @update:value="$emit('update:search', $event)"
      placeholder="Search tools..." size="small" clearable>
      <template #prefix>
        <n-icon :style="isDark ? 'color:#888' : 'color:#aaa'"><SearchOutline /></n-icon>
      </template>
    </n-input>
  </div>
  ...
  <div class="flex-1 overflow-y-auto py-2">
    <template v-for="cat in filteredCategories" :key="cat">
      <div class="px-3 py-1.5 text-xs font-semibold uppercase tracking-wider"
      ...
      <router-link
        ...
        :style="route.path === tool.path
          ? (isDark ? 'background:#2d2d35;color:#818cf8' : 'background:#eef2ff;color:#6366f1')
          : (isDark ? 'color:#ccc' : 'color:#444')"
        @click="$emit('navigate')"
      >
    ...
    <n-empty v-if="filteredTools.length === 0" description="No tools found" size="small" class="mt-8" />
  </div>
</template>
